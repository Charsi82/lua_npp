...
int Lua_print(lua_State* L)
{
	...
	while (args > 0 )
	{
		AddStr((char*)luaL_tolstring(L, -args, NULL));
		lua_pop(L, 1);
		AddStr(" ");
		args--;
	}
...
int Lua_MessageBox(lua_State* L)
{
	...
	TCHAR txt[MAX_PATH] = L"";
	TCHAR caption[MAX_PATH] = L"";

	SysUniConv::MultiByteToUnicode(txt, MAX_PATH, sz_txt);
	SysUniConv::MultiByteToUnicode(caption, MAX_PATH, sz_caption);

	int result = MessageBox(NULL, txt, caption, wnd_type);
